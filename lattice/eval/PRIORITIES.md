...
| M19 | `graph.py`, `embed.py`, `db.py` | **Semantic similarity edges** (optional, `LATTICE_DENSE_SEEDS=1` required): at write time, persist top-3 cosine neighbors per atom as `semantically_similar` edges using existing embed matrix. BFS then traverses vocabulary mismatches structurally rather than re-querying dense index at runtime. Cap at top-3 to bound edge count (max 3N edges). | Vocabulary mismatch ("running" ↔ "exercise") is a real product problem. Dense seeds already solve it at query time; persisted edges make it part of the graph structure, improving BFS recall for all downstream paths. Optional — off for Ollama users without embed. |
...
| M19 | Semantic similarity edges (optional) | — | Persist top-3 dense cosine neighbors per atom as graph edges at write time. BFS traverses vocab mismatches structurally. Requires `LATTICE_DENSE_SEEDS=1`. |
