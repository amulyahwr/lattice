...
| ~~M21~~ ✅ | `selection.py` | **Source-dominant POINTED mode** — file-gated `_source_dominance` + `_top_source_is_file` check. When probe diversity > 1 AND the dominant source is a file-based source (pdf:, docx:, etc.) AND dominance ≥ `LATTICE_POINTED_DOMINANCE` (default 0.7), stay POINTED. UUID/session sources bypass the check entirely. **p46-m21 (naive, no file gate): 68.3% (−4.5pp vs p44)** — naive dominance fired on LongMemEval UUID sessions, cutting multi-session atoms (same failure mode as p39). **p47-m21-v2 (file-gated): 72.8% (0pp vs p44 baseline)** — file gate correctly skips UUID sessions; all per-category deltas within ±1 SE noise. **Shipped.** | Binary probe over-expands on structurally similar multi-document file corpora (two CVs, two trip reports). Any query with a term common to both documents ("colleges", "skills", "work") flips to EXPANSION even when the proper noun anchor ("Alex Chen") makes intent unambiguous. File-gate distinguishes discrete documents (pdf:, docx:) from temporal session slices (UUID, chat, lc) — dominance check only meaningful for the former. |
...
| M16 | `graph.py`, `privacy.py`, `ingest.py` | **Named entity graph nodes** (optional, post-M7): NER at ingest adds `entity:PERSON` and `entity:ORG` nodes to graph; BFS expands from them. NER infrastructure shared with STORY-033 (`EntityRedactor` in `privacy.py`) — build once, reuse here. **Depends on M7** — entity nodes must follow hub design to avoid p21-style over-connection flood. Off by default; `LATTICE_NER_ENRICH=1` to enable. | Selection vocab mismatch for person/org queries ("Alex's skills" scores 0 on BM25 if atoms say only subjects). Entity nodes enable BFS expansion without touching BM25. Not for ingest attribution — that is handled by the ingest.py `_SYSTEM` named person prompt rule. |
...
| p46-m21 | **M21 naive**: `_source_dominance ≥ 0.7` → POINTED, no file gate. Reused p42 lattice dirs. **Reverted.** | **68.3%** | −4.5pp vs p44. Naive dominance probe fires on LongMemEval UUID session IDs — one session dominates BM25 seeds → POINTED cuts cross-session atoms needed for multi-session+temporal. Same failure as p39. File gate required. |
| p47-m21-v2 | **M21 file-gated**: dominance check only when `_top_source_is_file()` (pdf:, docx:, etc.). UUID/chat/lc sources bypass check. `LATTICE_POINTED_DOMINANCE=0.7`. Reused p42 lattice dirs. | **72.8%** | 0pp vs p44. All per-category deltas within ±1 SE stochastic noise. Product fix confirmed: two-CV queries stay POINTED; LongMemEval multi-session unaffected. **Shipped.** |
| p48-baseline-check | **Baseline verification**: cross-channel consistency release (multi-supersession, PII fast path, superseded atom filter in selection, BM25 apostrophe normalization, M21 file-gate). Reused p42 lattice dirs. `LATTICE_DENSE_SEEDS=1`, `LATTICE_DENSE_TOP_K=20`. | **70.0%** | −2.8pp vs p44 — within ±3.6% SE for n=180. **All 4 regressions are synthesis stochasticity**: bad rounding ("2.5 years → 3"), verbosity (extra car maintenance context buried the $65 answer), off-by-one date math (10 vs 11 years), judge borderline on verbosity. Zero superseded atoms in any regressed question's pack — superseded filter confirmed not the cause. **Baseline of 72.8% stands. Code changes are clean.** Selected hit 99.4% (vs ~99% p44) — marginal improvement from superseded filter. |

## Category Tracker (last 10 runs)

Note: phi4-mini-judge throughout. p19+ use qwen3-8b-ingest. p25 uses gpt-4o-mini ingest.

| Category | p34 (S-dense) | p42 (M17+dense) | p43 (decay sort) | p44 (K=20) | p48 (verify) |
| --- | --- | --- | --- | --- | --- |
| overall | 67.8% | 69.4% | 69.4% | **72.8%** | 70.0% |
| single-session-preference | 50.0% | 56.7% | 63.3% | **70.0%** 🎉 | 70.0% |
| temporal-reasoning | 46.7% | 56.7% | 46.7% | **60.0%** | 56.7% |
| multi-session | 66.7% | **76.7%** | 70.0% | 70.0% | 60.0% |
| knowledge-update | **70.0%** | 56.7% ⚠️ | **70.0%** | 73.3% | 70.0% |
| single-session-user | 83.3% | **93.3%** | 93.3% | 90.0% | 90.0% |
| single-session-assistant | **90.0%** | 76.7% | 73.3% | 73.3% | 73.3% |
