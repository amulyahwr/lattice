...
| `lattice/web/app.py` | FastAPI web UI. Serves static files from `lattice/web/static/` via `StaticFiles`. Routes: `GET /` → `index.html`, `POST /api/query` (streaming SSE synthesis), `POST /api/answer` (blocking JSON synthesis for Telegram — returns `{ok, answer, atom_count}`), `GET /api/usage/summary` (streak, today/7-day counts, avg latency — all channels write to `LATTICE_DIR/usage.jsonl` via `_record_usage()`), `GET /api/atoms/recent`, `POST /api/feedback`, `POST /api/ingest`. UI: "Save session" button, streak badge ("N days deep") in header (visible after first recall). Feedback thumbs shown only when `atom_count ≤ 1`. |
...
- Next: STORY-029 (Memory Sparks) → STORY-030 (streak depth + grace day) → STORY-031 (weekly report) → STORY-032 (rediscovery highlight) → VS Code extension
