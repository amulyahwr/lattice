...
| `lattice/web/app.py` | FastAPI web UI. Routes: `GET /` → `index.html`, `POST /api/query` (streaming SSE synthesis; accepts `conversation_history: list[{question,answer}]` + `session_id`; runs `is_followup()` → `reformulate()` when true; writes Q&A turn to `chat.jsonl`; SSE `atoms` event includes `context_reset: bool`), `POST /api/answer` (blocking JSON synthesis — same conversation_history support; Telegram path), `GET /api/chat/recent?session_id=&limit=` (last N Q&A turns for page-reload restore), `POST /api/ingest` (text), `POST /api/ingest-file` (multipart — accepts any file type via `extract_file_text()`; returns `atoms_new`, `atoms_updated`, `duplicates_skipped`), `GET /api/usage/summary`, `GET /api/usage/weekly`, `GET /api/atoms/recent`, `GET /api/topic/depth`, `POST /api/feedback` (stores `atom_ids`, `dismissed_atom_ids`, `citation_map` atom_id→display number, plain-text `answer`), `GET /api/health`. `chat.jsonl`: every Q&A appended with `{ts, session_id, question, reformulated_query?, answer, atom_ids, channel}`. UI: multi-file upload, spark cards + ghost queries in empty state, "Save session" button, streak badge, milestone cards, rediscovery amber glow, scrollable sources panel with per-source ✕ dismiss, thumbs feedback, PII badge. |
...
| `lattice/conversation.py` | Multi-turn query reformulation. `is_followup(query)` — detects anaphoric/short follow-ups via phrase fast-path (`_FOLLOWUP_PHRASES`), pronoun tokens (`_ANAPHORIC_TOKENS`), and no-proper-noun + short heuristic. `reformulate(query, history, cfg)` — single LLM call (model: `REFORMULATION_MODEL` → `INGEST_MODEL` → `LLM_MODEL`); PII redact/restore on history + query before LLM call; fallbacks on empty/identical/too-long response. Tests mock `lattice.conversation.complete`. |
