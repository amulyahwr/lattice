...
| `lattice/cli.py` | `lc` entry point. `lc "decided X because Y"` captures via `DaemonClient().ingest()` over Unix socket; exits 1 with actionable message if daemon not running. `lc status` reads `LatticeDB` directly (no daemon needed) and prints memory count + top-5 topics + streak ("N days deep") from `usage.jsonl`. |
| `lattice/telegram_bot.py` | Telegram capture bot. Independent launchd service (`dev.lattice.telegram.plist`). Polls via `python-telegram-bot` (`bootstrap_retries=-1`, `timeout=30`). Auto-detects intent (`_classify`): question → recall, statement → capture, ambiguous → clarification prompt. Capture: `DaemonClient().ingest()`. Recall: `POST /api/answer` (daemon has LLM env vars; bot does not) → strips `[label][src:id]` citations, appends `Sources:\n· label` footer; appends rediscovery note if any cited atom ≥30 days old; prompts 👍/👎 feedback when `atom_count ≤ 1`; feedback posted to `POST /api/feedback`. `/save` ingests session history. `/status` shows memory count + streak + grace day. `/start` shows 3 suggested questions from recent atom subjects (warm empty-state message when no atoms). Milestone moments (Day 1/7/14/30) prepended before first recall reply of milestone day. Daemon-down: inbox fallback with chat_id in filename; daemon drains on restart and sends follow-up. Commands: `/ask`, `/save`, `/status`, `/start`. Requires `uv sync --group telegram`. |
...
| `lattice/web/app.py` | FastAPI web UI. Serves static files from `lattice/web/static/` via `StaticFiles`. Routes: `GET /` → `index.html`, `POST /api/query` (streaming SSE synthesis), `POST /api/answer` (blocking JSON synthesis for Telegram — returns `{ok, answer, atom_count, atoms}`; `atoms` includes `ingested_at` for rediscovery highlight), `GET /api/usage/summary` (streak, grace_day_active, atom_count, today/7-day counts, avg latency — `_compute_streak_with_grace()` handles grace day logic; grace day sentinel written to `usage.jsonl` as `{type:"grace_day_used"}`), `GET /api/atoms/recent`, `POST /api/feedback`, `POST /api/ingest`. UI: spark cards + ghost queries in empty state, "Save session" button, streak badge ("N days deep" / "N days deep · rest day" / "30 days deep 🎯") in header. Milestone cards (Day 1/7/14/30) shown in chat area after first query of milestone day; dismissed via localStorage key. Rediscovery amber glow on citations from atoms ≥30 days old. Feedback thumbs shown only when `atom_count ≤ 1`. |
...
| `lattice/synthesis.py` | Tool-calling agent via OpenAI-compat SDK (uses `make_llm_client()` from `llm.py`). Model from `SYNTHESIS_MODEL` (falls back to `LLM_MODEL`). Tools: `date_diff`, `sum_numbers`. `synthesize()` → `SynthesisResult`. `stream_synthesis()` → SSE generator used by the web UI. `_is_no_answer(text)` — checks for `<<NO_INFO>>` sentinel (prompt instruction to LLM) first, then fuzzy regex fallback for LLMs that paraphrase; replaces entire answer with warm short phrase. Sentinel approach prevents the LLM from copying the display phrase into real answers. Ollama-specific `extra_body` applied only when `LLM_PROVIDER=ollama`. |
...
- ✅ Telegram feedback (STORY-027) — 👍/👎 prompt after recall when `atom_count ≤ 1`; reason collection; posts to `feedback.jsonl`
